...
export class Image {
  ...
  onClosing(fancybox) {
    ...
    if (
      ...
      this.canZoom(fancybox.getSlide())
    ) {
      this.zoomOut()
    }
  }
  ...
  setContent(slide) {
    ...
    if (shouldWrap) {
      ...
      $wrap.classList.add(
        typeof shouldWrap === 'string' ? shouldWrap : 'fancybox__image-wrap'
      )
  ...
  onImageStatusChange(slide) {
    ...
    slide.$content.addEventListener(
      'click',
      (event) => this.onClick(slide, event),
      { passive: false }
    )
  ...
  getZoomInfo(slide) {
    ...
    if (opacity === 'auto') {
      opacity =
        Math.abs(thumbWidth / thumbHeight - contentWidth / contentHeight) > 0.1
    }
  ...
  canZoom(slide) {
    ...
    if (
      !fancybox.option('Image.zoom') ||
      fancybox.option('Image.fit') !== 'contain'
    ) {
      return false
    }
    ...
    if (this.fancybox.option('Image.ignoreCoveredThumbnail')) {
      const visibleTopLeft =
        document.elementFromPoint(rect.left + 1, rect.top + 1) === $thumb
      const visibleBottomRight =
        document.elementFromPoint(rect.right - 1, rect.bottom - 1) === $thumb
    ...
    if (opacity === true) {
      Panzoom.on('afterTransform', (panzoom) => {
        if (slide.state === 'zoomIn' || slide.state === 'zoomOut') {
          panzoom.$content.style.opacity = Math.min(
            1,
            1 - (1 - panzoom.content.scale) / (1 - scale)
          )
        }
      })
    }
  ...
  handleCursor(slide) {
    ...
    if (panzoom && clickAction === 'toggleZoom') {
      const canZoomIn =
        panzoom &&
        panzoom.content.scale === 1 &&
        panzoom.option('maxScale') - panzoom.content.scale > 0.01

...
export class Toolbar {
  ...
  onSettle() {
    ...
    if (slideshow && slideshow.isActive()) {
      if (
        fancybox.getSlide().index === fancybox.Carousel.slides.length - 1 &&
        !fancybox.option('infinite')
      ) {
  ...
  onDone(fancybox, slide) {
    ...
    if (slide.index === fancybox.getSlide().index) {
      ...
      if (slideshow && slideshow.isActive()) {
        if (
          !fancybox.option('infinite') &&
          slide.index === fancybox.Carousel.slides.length - 1
        ) {
          slideshow.deactivate()
        } else {
          slideshow.setTimer()
  ...
  build() {
    ...
    for (const key of this.fancybox.option('Toolbar.display')) {
      ...
      if (
        ['counter', 'next', 'prev', 'slideshow'].includes(id) &&
  ...
  update() {
    ...
    const src =
      slide.downloadSrc ||
      (slide.type === 'image' && !slide.error ? slide.src : null)

    for (const $el of this.fancybox.$container.querySelectorAll(
    ...
    const canZoom =
      panzoom && panzoom.option('maxScale') > panzoom.option('baseScale')

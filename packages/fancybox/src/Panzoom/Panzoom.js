...
import {
  ...
  getMidpoint,
  getDistance
} from '../shared/utils/PointerTracker.js'
...
} from '../shared/utils/getDimensions.js'
...
export class Panzoom extends Base {
  ...
  initLayout() {
    ...
    const $content =
      this.option('content') || $container.querySelector('.panzoom__content')
    ...
    let $viewport =
      this.option('viewport') || $container.querySelector('.panzoom__viewport')
  ...
  zoomWithWheel(event) {
    ...
    const delta = Math.max(
      ...
      Math.min(
        1,
        -event.deltaY || -event.deltaX || event.wheelDelta || -event.detail
      )
    )
    ...
    if (
      ...
      this.changedDelta = 0
      newScale = Math.max(
        Math.min(newScale, this.option('maxScale')),
    ...
    const pointerTracker = new PointerTracker(this.$container, {
      start: (pointer, event) => {
        ...
        if (!pointerTracker.currentPointers.length) {
          ...
          if (
            this.option('textSelection') &&
            getTextNodeFromPoint(target, pointer.clientX, pointer.clientY)
          ) {
            return false
          }
        }
      ...
      move: (previousPointers, currentPointers, event) => {
        ...
        if (
          ...
          (!this.option('zoom') || this.option('pinchToZoom') === false)
        ) {
          return
        }
        ...
        const scaleDiff =
          prevDistance && newDistance ? newDistance / prevDistance : 1
        ...
        if (axisToLock && !this.lockAxis) {
          ...
          const angle = Math.abs(
            (Math.atan2(this.dragOffset.y, this.dragOffset.x) * 180) / Math.PI
          )
  ...
  initObserver() {
    ...
    this.resizeObserver = new ResizeObserver(() => {
      ...
      this.updateTimer = setTimeout(() => {
        ...
        if (
          Math.abs(rect.width - this.container.width) > 1 ||
          Math.abs(rect.height - this.container.height) > 1
        ) {
          if (this.isAnimating()) {
  ...
  updateMetrics(silently) {
    ...
    if (contentIsZoomable) {
      if (
        Math.abs(origWidth - contentRect.width) > 0.1 ||
        Math.abs(origHeight - contentRect.height) > 0.1
      ) {
    ...
    if (
      contentIsImage &&
      contentIsZoomable &&
      typeof this.options.maxScale !== 'function'
    ) {
      const maxScale = this.option('maxScale')
  ...
  toggleZoom(props = {}) {
    ...
    const scale =
      this.content.scale > baseScale + (maxScale - baseScale) * 0.5
        ? baseScale
        : maxScale
  ...
  setEdgeForce() {
    ...
    const { boundX, boundY } = this.getBounds(

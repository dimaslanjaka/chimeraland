...
export class Carousel extends Base {
  ...
  initLayout() {
    ...
    this.$viewport =
      ...
      this.$container.querySelector(`.${prefix}${classNames.viewport}`)
    ...
    this.$track =
      this.option('track') ||
      this.$container.querySelector(`.${prefix}${classNames.track}`)
  ...
  initSlides() {
    ...
    const elems = this.$viewport.querySelectorAll(
  ...
  updateMetrics() {
    ...
    this.slides.forEach((slide, index) => {
      const $el = slide.$el
      const slideWidth =
        slide.isDom || !lastSlideWidth
          ? this.getSlideMetrics($el)
          : lastSlideWidth
    ...
    let viewportWidth = Math.max(
      ...
      round(this.$track.getBoundingClientRect().width)
    )
  ...
  getSlideMetrics(node) {
    ...
    let width = Math.max(
      node.offsetWidth,
      round(node.getBoundingClientRect().width)
    )
  ...
  manageSlideVisiblity() {
    ...
    let currentX = this.Panzoom
      ? this.Panzoom.content.x * -1
      : this.pages.length
    ...
    this.slides.forEach((slide) => {
      ...
      const insideCurrentInterval =
        slide.left + slide.width > leftBoundary && slide.left < rightBoundary
      ...
      const insidePrevInterval =
        infinite &&
        slide.left + slide.width > leftBoundary &&
        slide.left < rightBoundary
      ...
      if (insidePrevInterval || insideCurrentInterval || insideNextInterval) {
        ...
        if (
          slide.left + slide.width > currentX &&
          slide.left <= currentX + viewportWidth + paddingRight
        ) {
          hasDiff = 0
        }
      } else {
    ...
    this.slides.forEach((slide, index) => {
      ...
      if (slide.$el) {
        ...
        slide.$el.style.left =
          Math.abs(updatedX) > 0.1
            ? `${nextPos + slide.hasDiff * contentWidth}px`
            : ''
  ...
  createSlideEl(slide) {
    ...
    this.$track.insertBefore(
      div,
      refSlide && refSlide.$el
        ? refSlide.index < slide.index
          ? refSlide.$el.nextSibling
          : refSlide.$el
        : null
    )
  ...
  onTouchEnd(panzoom, _event) {
    ...
    if (dragFree) {
      const [, nextPageIndex] = this.getPageFromPosition(
        panzoom.transform.x * -1
      )
  ...
  slideToClosest(params = {}) {
    let [, nextPageIndex] = this.getPageFromPosition(
      this.Panzoom.content.x * -1
    )
  ...
  getPageFromPosition(xPos) {
    ...
    let slide = this.slides.find(
      (slide) => slide.left <= xPos && slide.left + slide.width > xPos
    )

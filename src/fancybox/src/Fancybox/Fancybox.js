...
class Fancybox extends Base {
    ...
    this.Carousel = new Carousel(
      ...
      extend(
        ...
        {},
        {
          ...
          on: {
            '*': (name, ...details) =>
              this.trigger(`Carousel.${name}`, ...details),
  ...
  onTouchEnd(panzoom) {
    ...
    if (
      Math.abs(distanceY) >= 150 ||
      (Math.abs(distanceY) >= 35 && panzoom.dragOffset.time < 350)
    ) {
      if (this.option('hideClass')) {
        this.getSlide().hideClass = `fancybox-throwOut${
          panzoom.content.y < 0 ? 'Up' : 'Down'
        }`
      }
    ...
    if ($backdrop) {
      ...
      const opacity =
        yPos < 1
          ...
          : Math.max(
              0.33,
              Math.min(1, 1 - (yPos / panzoom.content.fitHeight) * 1.5)
            )
  ...
  getSlide() {
    ...
    const page =
      carousel.page === null ? carousel.option('initialPage') : carousel.page
  ...
  focus(event) {
    ...
    const $currentSlide =
      currentSlide.state === 'done' ? currentSlide.$el : null
    ...
    const allFocusableElems = Array.from(
    ...
    for (let node of allFocusableElems) {
      ...
      const isNodeInsideCurrentSlide =
        $currentSlide && $currentSlide.contains(node)
      const isNodeOutsideCarousel = !this.Carousel.$viewport.contains(node)

      if (
        isNodeVisible &&
        (isNodeInsideCurrentSlide || isNodeOutsideCarousel)
      ) {
        ...
        if (
          ...
          (!$firstEl &&
            isNodeInsideCurrentSlide &&
            !node.classList.contains('carousel__button'))
        ) {
          $firstEl = node
        }
      } else {
        ...
        node.dataset.origTabindex =
          node.dataset.origTabindex === undefined
  ...
  setContent(slide, html, opts = {}) {
    ...
    if (
      ...
      getComputedStyle($content).getPropertyValue('display') === 'none'
    ) {
      $content.style.display =
        slide.display || this.option('defaultDisplay') || 'flex'
    }
  ...
  close(event) {
    ...
    if (this.state === 'closing') {
      const hideClass =
        currentSlide.hideClass === undefined
          ? this.option('hideClass')
          : currentSlide.hideClass
  ...
  destroy() {
    ...
    const $trigger = this.option('placeFocusBack')
  ...
  static fromEvent(event, options = {}) {
    ...
    if (
      ...
      triggerGroupName =
        eventTarget &&
        eventTarget.dataset &&
        eventTarget.dataset.fancyboxTrigger
    }

    if (triggerGroupName) {
      const triggerItems = document.querySelectorAll(
        `[data-fancybox="${triggerGroupName}"]`
      )
      const triggerIndex = parseInt(eventTarget.dataset.fancyboxIndex, 10) || 0

      eventTarget = triggerItems.length
        ? triggerItems[triggerIndex]
        : eventTarget
    }
      ...
      .some((opener) => {
        ...
        try {
          if (
            ...
            (typeof opener === 'string' || opener instanceof String)
          ) {
  ...
  static fromOpener(opener, options = {}) {
    ...
    const groupAttr =
      options.groupAttr === undefined ? 'data-fancybox' : options.groupAttr
    const groupValue =
      groupAttr && target ? target.getAttribute(`${groupAttr}`) : ''

    if (!target || groupValue || groupAll) {
      const $root =
        options.root || (target ? target.getRootNode() : document.body)
    ...
    if (target && !groupAll) {
      if (groupValue) {
        items = items.filter(
          (el) => el.getAttribute(`${groupAttr}`) === groupValue
        )
      } else {
        items = [target]
      }
    ...
    if (
      currentInstance &&
      items.indexOf(currentInstance.options.$trigger) > -1
    ) {
      return false
    }
...
// Prepare plugins
for (const [_key, Plugin] of Object.entries(Fancybox.Plugins || {})) {

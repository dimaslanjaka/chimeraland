...
export class Html {
  ...
  processType(slide) {
    ...
    if (
      ...
      slide.thumb =
        slide.thumb || `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`
      ...
      slide.src = `//maps.google.${rez[1]}/?ll=${(rez[2]
        ...
        rez[4] && rez[4].indexOf('layer=c') > 0 ? 'svembed' : 'embed'
      }`
      ...
      slide.src = `//maps.google.${rez[1]}/maps?q=${rez[2]
        .replace('query=', 'q=')
  ...
  loadInlineContent(slide) {
    ...
    if ($content) {
      if (slide.type === 'clone' || $content.$placeHolder) {
        ...
        attrId = attrId
          ? `${attrId}--clone`
          : `clone-${this.fancybox.id}-${slide.index}`
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (fancybox.state === 'ready') {
          ...
          if (xhr.status === 200) {
            ...
            fancybox.setError(
              slide,
              xhr.status === 404 ? '{{AJAX_NOT_FOUND}}' : '{{AJAX_FORBIDDEN}}'
            )
          }
        }
      }
    ...
    if (slide.preload !== false && slide.autoSize !== false && parentStyle) {
      try {
        const compStyles = window.getComputedStyle($parent),
          paddingX =
            parseFloat(compStyles.paddingLeft) +
        ...
        this.fancybox.setContent(
          ...
          this.fancybox
            ...
            .replace(
              '{{format}}',
              slide.format ||
                (slide.html5video && slide.html5video.format) ||
                ''
            )
  ...
  playVideo(slide) {
    ...
    if (
      slide.type !== 'video' ||
      !(slide.$iframe && slide.$iframe.contentWindow)
    ) {
      return
    }
    ...
    const poller = () => {
      if (
        ...
        if (slide.$iframe.isReady) {
          ...
          if (command) {
            slide.$iframe.contentWindow.postMessage(
